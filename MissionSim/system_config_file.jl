...
# CONSTANTS --------------------------------------------------------------------#
    ...
    _J = _J .* [0.86 1.0 1.2] #rand(Uniform(0.75, 1.25), 3)
...
# STATE ------------------------------------------------------------------------#
    ...
    # """   ISS   """
    println("Using ISS Orbit")
    ecc = 0.0001717 + 0.000001 * randn()
    inc = 51.6426 + randn()
    Ω   = 178.1369 + randn()
    ω   = 174.7410 + randn()
    M   = 330.7918 + 110 + randn()
    ...
    q0 = q0 / norm(q0)
    w0 = 2.5 .* deg2rad.([10, 2, -7.2]) # HOW DO I SELECT REASONABLE VALUES HERE...?    
...
# DIODES -----------------------------------------------------------------------#
    ...
    _elev_angles = [(-pi/4); (pi/4);  0.0;    0.0;    (pi/4); (-pi/4)] 
    _azi_angles  = [0.0;      pi;     (pi/2); (-pi/2); 0.0;    pi]   
    # _elev_angles = [(-pi/4);  (pi/4);      0.0;      0.0; (pi/4);  (-pi/4)] 
    # _azi_angles  = [(pi/4); (5*pi/4); (3*pi/4); (7*pi/4); (pi/4); (5*pi/4)]   

    println("Diodes rotated 45° around ŷ")#  and ẑ")
...
# GENERAL ----------------------------------------------------------------------#
    _dt = 0.25 #1.0  # (s)    
    @info("Running at 4 Hz!")
    _T = round(2.25 * orbit_period(oe0[1]) / _dt)  # Run for 2.25 orbits
    @show _T
    _epc = Epoch(2021, 9, 1, 11, 0, 0, 0.0); # Initial time for sim
    _max_sim_length = Int(_T)
    _ds_rate = Int(round(120/_dt))
