...
struct MEKF_DATA{T}
    ...
    function MEKF_DATA(; N = 6, σ_mag = deg2rad(3.0), σ_cur = 0.1,
                            ...
                            σC = 1e-4, σα = deg2rad(0.01), σϵ = deg2rad(0.01),
        ...
        W = Diagonal( [σ_gyro * ones(3); σ_bias * ones(3); σs * ones(3); σζ * ones(3); 10 * σ_bias * ones(3); σC * ones(N); σα * ones(N); σϵ * ones(N)].^2 )
        Wchol = chol(Matrix(W)) 
