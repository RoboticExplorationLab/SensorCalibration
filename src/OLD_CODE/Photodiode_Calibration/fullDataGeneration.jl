...
# Currently multiplying bias by zero for testing (same with β₀ in config file)
function dynamics(x, param, t) 
    ...
    xdot[14:16] = 0.01 * σ_β * randn(3)                # bias is just a random walk, so β = β + η
...
println("NOISE MATRIX is just I(3)")
...
display(plot(states[11:13,:]', labels = ["wx" "wy" "wz"], xlabel = "Time ($dt s)", ylabel = "Angular Velocity (rad/$tscale s)", title = "W"))
display(plot(states[14:16,:]', labels = ["βx" "βy" "βz"], xlabel = "Time ($dt s)", ylabel = "Bias (??)", title = "Biases"))
