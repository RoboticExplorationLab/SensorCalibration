function mekf(x0, P0, W, V, rN, whist, yhist, dt, numDiodes, eclipse, pos, epc)
    # Runs a multiplicative extended Kalman filter. 
    ...
    @showprogress "MEKF" for k = 1:(size(yhist,2)-1) 
        ...
        #     sN = rN[1:3,k];
        #     V_sun = V[1:3, 1:3]; 
        #     yp_sun, C_sun = sun_measurement(x_p, sN, numDiodes) 
        ...
        if eclipse[k] > 0   #  Diode Current Measurement
            ...
            time = (k-1) * dt + epc
            sN = rN[1:3,k]
            yp_cur, C_cur = current_measurement(x_p, sN, numDiodes, eclipse[k], pos[:,k], time)
