# Nonlinear (Gauss-Newton), includes current measurements
...
function measurement(x,t)
    ...
    # R = I(3)
    b_fieldᴮ = R * b_field
    b_field_hat = (T_matrix*b_fieldᴮ) + bias + magFromCur;
...
m  = 9 + numCurrents      # Size of measurement matrix [pred, meas, current meas] 
...
R = Diagonal([prediction_noise*ones(3); measurement_noise*ones(3); measurement_noise*ones(3); current_noise*ones(numCurrents)].^2)
cholR = sqrt(R)
invcholR = inv(cholR)
...
# true_params = [s_a; s_b*sin(ρ); s_b*cos(ρ); 0; s_c*sin(λ); s_c*sin(ϕ)*cos(λ); s_c*cos(ϕ)*cos(λ);
#                 bias[:]; curCoef[:]]
...
currMeas = yVals[10:end, :]

function f(bm, cm, p)
    # Reshape p -> T, b, s 
    T_hat = params_to_matrix(p[1:6])
    bias_hat = p[7:9]
    curCoef_hat = p[10:end]
...
function residual(x)
    ...
    for i = 1:T 
        ...
        curr_meas = currMeas[:, i] # Should be [M x c = 6]

        J = -((B_exp_squared) - f(B_meas, curr_meas, x)) 
        # J = 0.5 * (B_exp_squared - f(B_meas, curr_meas, x))' * (B_exp_squared - f(B_meas, curr_meas, x))
...
A = [(y_pred[1,1]*I(3))   (y_pred[2,1]*I(3))[:, 2:3]   (y_pred[3,1]*I(3))[:, 3]    I(3)     currRow]
...
for i = 2:size(yVals)[2]
    ...
    row = [(y_pred[1,i]*I(3))   (y_pred[2,i]*I(3))[:, 2:3]   (y_pred[3,i]*I(3))[:, 3]    I(3)   currRow] 
    global A = [A; row]
end
...
T_hat = params_to_matrix(params[1:6])
# T_hat = reshape(params[1:9], 3, 3)'

bias_hat = params[7:9]
curCoef_hat = params[10:end]
curCoef_hat = reshape(curCoef_hat, size(curCoef))
...
y_cor = (T_hat^(-1))*((y_meas - magFromCur) .- bias_hat)
...
pltDif2 = plot( y_dif[1,:], color = :red, title = "Final Error (OG)")
