"""
    TEST FILE for comparing diode_calibration_sqrt.jl with sqrtMekfCalibrator.py 
        to verify the python implementation.

    Data is specified and then specific functions common to both files are run.
    A few full iterations of the MEKF are also run and compared
"""
...
function __init__() 
    ...
    import scipy.io 
    from sqrtMekfCalibrator import SqrtMekfCalibrator
...
σ_α = 2.0 #1.0 
σ_ϵ = 1.0 #0.3 
p = [σ_q * ones(3); σ_β * ones(3); σ_c * ones(num_diodes); σ_α*ones(num_diodes); σ_ϵ*ones(num_diodes)].^2
P₀ = diagm(p)
P₀ = chol(P₀)
...
##### BEGIN TESTS
if true 
    @testset "Function Subsets" begin
        # UPDATE JULIA 
        jl_data = new_diode_calib(albedo, sensors, SYSTEM, q, sat_est)
        ...
        c = [1, 1, 1, 1, 1, 1] 
        a = [0, pi, pi/2, -pi/2, 0, pi] 
        e = [-pi/4, pi/4, 0, 0, pi/4, -pi/4] 


        # CHOL 
        obj_py = py"SqrtMekfCalibrator"(c, a, e, SYSTEM, model, data)
        ...
        xp_jl, A_jl = prediction(est.sat_state, sensors.gyro, SYSTEM._dt, SYSTEM._num_diodes) 
        @test xp_py ≈ xp_jl
        ...
        ymag_jl, Cmag_jl = mag_measurement(est.sat_state, Bi_unit, SYSTEM._num_diodes)
        ...
        ycur_py, Ccur_py = obj_py.current_measurement(est.sat_state, si_unit)
        ymag_jl, Cmag_jl = current_measurement(est.sat_state, si_unit, SYSTEM._num_diodes,
                                                data["gps"], t, albedo)
        @test ycur_py ≈ ymag_jl 
        @test Ccur_py ≈ Cmag_jl
    end
end
...
@testset "Multiple Iterations" begin 
    for i = 1:10
        ...
        local sat_true, sat_est = generate_satellite()
        ...
        local sensors = fill_sensors()
        local q = get_random_quat() # 
        R = dcm_from_q(q)'

        local sᴮ = estimate_sun_vector(sensors, sat_est)
        local sᴵ = 1e10 * (R' * sᴮ) - sensors.gps

        local Bᴮ  = sensors.magnetometer
        local Bᴵ  = R' * Bᴮ #[33, 2.0, 11.3]

        local est = new_diode_calib(albedo, sensors, SYSTEM, q, sat_est)

        est.inertial_vecs = [sᴵ Bᴵ]'
        est.body_vecs = [sᴮ Bᴮ]'
        est.current_meas = sensors.diodes

        local t, t_py = get_random_epoch() #  Epoch(2021, 9, 1, 11, 0, 0, 0.0)
        ...
        # @testset "Full Iteration" begin 
            ...
            jl_data.body_vecs = est.body_vecs
            jl_data.time = t 
            jl_sat, data = estimate_vals(sat_est, jl_data)
            ...
            obj_py = py"SqrtMekfCalibrator"(c, a, e, SYSTEM, model, data)
            s1 = obj_py.update_estimate(data, sat_est)


            # COMPARISON
            @test obj_py.q ≈ jl_sat.state[1:4]
            @test obj_py.bias ≈ jl_sat.state[5:7]
            @test obj_py.calibration_values ≈ jl_sat.diodes.calib_values
            @test jl_sat.diodes.azi_angles  ≈ obj_py.azimuth_angles 
            @test jl_sat.diodes.elev_angles ≈ obj_py.elevation_angles
